...
namespace SafeEntry.Core.Utils
{
    ...
    {
        public static QRCodeModel GenerateQRCodeInvitation(EventInvitationDto request)
        {
            QRCodeModel qrCodeModel = new QRCodeModel();
            var sha256 = SHA256.Create();
            var shaData = sha256.ComputeHash(Encoding.UTF8.GetBytes(request.Email.ToString() + request.FirstName.ToString() + request.LastName.ToString()));
